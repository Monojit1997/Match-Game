...
class MainUi extends Component {
  ...
  getFilteredProject = () => {
    ...
    const filteredProject = imagesList.filter(
  ...
  checkMatchImage = id => {
    const {matchImagesrc, score, gameOver} = this.state
    if (matchImagesrc.id === id) {
      const randomNumber = Math.ceil(Math.random() * 20)
      this.setState(prevState => ({
        matchImagesrc: imagesList[randomNumber],
  ...
  render() {
    const {activeTab, matchImagesrc, score, gameOver, timer} = this.state
      ...
      <div className="bg-container">
        ...
        {gameOver || timer < 1 ? (
          ...
          <div>
            ...
            <ul className="thumbnailUrl-container">
